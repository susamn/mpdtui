...
- **Now Playing bar** — a green/red play-pause-stop glyph, artist (bold
  sky blue, rendered fullwidth for visual size) - title (bold WhatsApp
  green, also fullwidth), live progress (cyan bar), volume (colored
  along a green-to-red gradient by level), repeat/random/single/consume
  flags (bold, green when on/red when off), and the current track's
  local rating and play count (when `track_metadata` is active) --
  updated instantly via MPD's `idle` protocol (stays in sync even when
