...
// Genre/Year, anchored to the bottom-right quadrant of the Queue panel
// (splitting that panel into a 2x2 grid) rather than centered on the full
// screen like the other overlays. It embeds *tview.TextView and overrides
// Draw so its position is recomputed from the Queue table's live rect on
// every frame -- the same "read another primitive's current rect at draw
// time" trick albumArtPanel.draw uses for the Kitty image, which is what
// makes this track a terminal resize without any extra wiring.
...
// positionOverQueue sets the card's own rect to the bottom-right quadrant
// of the Queue table's current rect. Split out from Draw so the
// positioning math is testable without a real tcell.Screen.
func (c *trackInfoCard) positionOverQueue() {
	x, y, w, h := c.app.queue.table.GetRect()
	c.SetRect(quadrantRect(x, y, w, h))
}
...
func (c *trackInfoCard) render(song mpdclient.Song) {
	...
	lines := []string{
		fmt.Sprintf("[::b]Track:[-:-:-]  %s", track),
		fmt.Sprintf("[::b]Album:[-:-:-]  %s", song.Album),
		fmt.Sprintf("[::b]Artist:[-:-:-] %s", song.Artist),
		fmt.Sprintf("[::b]Genre:[-:-:-]  %s", song.Genre),
		fmt.Sprintf("[::b]Year:[-:-:-]   %s", year),
	}
...
// takes focus while open; Esc (handled globally in overlay mode) closes it
// and restores whichever panel was focused before 'i' was pressed, same as
// every other overlay.
